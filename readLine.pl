...
/* additionally export readLineNoDecap/1 */
...
/* added to read line without decapitalization, for user's name parsing */
readLineNoDecap(WordList):-
   nl, write('> '),
   readWords(Words),
   checkWordsNoDecap(Words,WordList).
...
/* for readLineNoDecap/1 */
checkWordsNoDecap([],[]):- !.

checkWordsNoDecap([''|Rest1],Rest2):-
   checkWordsNoDecap(Rest1,Rest2).

checkWordsNoDecap([Atom|Rest1],[Atom2|Rest2]):-
   name(Atom,Word1),
   convertWordNoDecap(Word1,Word2),
   name(Atom2,Word2),
   checkWordsNoDecap(Rest1,Rest2).
...
convertWordNoDecap([Capital|Rest1],[Capital|Rest2]):-
   Capital > 64, Capital < 91, !,
   convertWordNoDecap(Rest1,Rest2).

convertWordNoDecap([Number|Rest1],[Number|Rest2]):-
   Number > 47, Number < 58, !,
   convertWordNoDecap(Rest1,Rest2).

convertWordNoDecap([Weird|Rest1],Rest2):-
   (Weird < 97; Weird > 122), !,
   convertWordNoDecap(Rest1,Rest2).

convertWordNoDecap([Char|Rest1],[Char|Rest2]):-
   convertWordNoDecap(Rest1,Rest2).
